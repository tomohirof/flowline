...
- **エッジ数と同じ数のノード障害物**（`nodeObstacles` に 10×10 グリッドの Bbox を生成）
- from/to に共有エンドポイントを持たないペア（`node_i → node_{i+1 mod n}`）
- 10×10 グリッド上に座標展開、`ArrowConfig = { hw: 50, hh: 25, rh: 100 }`

| E   | ops/sec | mean (ms) | p99 (ms) |
| --- | ------- | --------- | -------- |
| 10  | 238,246 | 0.0042    | 0.0050   |
| 50  | 17,179  | 0.0582    | 0.1523   |
| 100 | 4,874   | 0.2052    | 0.2875   |
| 200 | 1,307   | 0.7650    | 0.8606   |

**結論**: E=200 でも mean ~0.77 ms / p99 ~0.86 ms と、1 フレーム（16.67 ms）予算の **5% 未満** で完了。
E² 増加（10→200 で 182×）はおおむね理論通りで、段階1 のターゲット規模（実運用 E ≤ 100 想定）には十二分。
段階2/3 の範囲インデックス導入は **E > 500 程度** まで保留して問題ない。

> 注: 合成シナリオは共有エンドポイント無し（最悪ケース寄り）で、各エッジで `foreignSegments` が
> ほぼフルに蓄積される。`nodeObstacles` をエッジ数分の Bbox で埋めて現実シナリオを模倣しており、
> 実フローではこれより軽くなる傾向。
