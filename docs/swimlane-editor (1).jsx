...
export default function SwimlanEditor() {
  ...
  const ct = useCallback(
    (li, ri) => ({ x: laneX(li) + LW / 2, y: TM + HH + ri * RH + RH / 2 }),
    [lanes.length, G],
  )
      ...
      for (let li = 0; li < lanes.length; li++)
        for (let ri = 0; ri < rows.length; ri++)
          if (ky(lanes[li].id, rows[ri].id) === dragOver) {
            moveTask(dragging.key, { lid: lanes[li].id, rid: rows[ri].id, key: dragOver })
            setDragging(null)
            setDragOver(null)
            return
          }
  ...
  const aPath = (arrow) => {
    ...
    if (Math.abs(dx) < 2 || Math.abs(dy) < 2) {
      ...
      const sV = Math.abs(s.y - f.y) > Math.abs(s.x - f.x),
        eV = Math.abs(e.y - t.y) > Math.abs(e.x - t.x)
      if (sV && eV) {
        const my = (s.y + e.y) / 2
        ...
        <>
          ...
          <PanelSection label="メモ" T={T}>
            <PanelInput
              value={notes[selTask] || ''}
              placeholder="メモを追加…"
              onChange={(v) => setNotes((p2) => ({ ...p2, [selTask]: v }))}
              T={T}
            />
          </PanelSection>
                  ...
                  {editLane === lane.id ? (
                    <foreignObject x={x + 32} y={TM + 9} width={LW - 44} height={28}>
                      <input
                        ...
                        onKeyDown={(e) => e.key === 'Enter' && setEditLane(null)}
                        style={{
                          width: '100%',
                          background: T.inputBg,
                          border: `1px solid ${T.inputBorder}`,
            ...
            {lanes.map((lane, li) =>
              rows.map((row, ri) => {
                ...
                const oi = order.indexOf(k),
                  ...
                  isConnTgt = connectFrom && connectFrom !== k
                        ...
                        {editNote === k ? (
                          ...
                          >
                            <input
                              ref={inputRef}
                              value={note}
                              onChange={(e) => setNotes((p2) => ({ ...p2, [k]: e.target.value }))}
