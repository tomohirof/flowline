...
describe('getProjectRole', () => {
  ...
  beforeEach(() => {
    ...
    db.prepare(
      `INSERT INTO projects (id, user_id, name) VALUES ('proj-1', 'owner-1', 'P1')`,
    ).run()
    db.prepare(
...
describe('canAccessFlow', () => {
  ...
  beforeEach(() => {
    ...
    db.prepare(
      `INSERT INTO flows (id, user_id, title) VALUES ('flow-2', 'owner-1', 'F2')`,
    ).run()
    db.prepare(
  ...
  it('grants edit access but not delete access to project editors', async () => {
    expect(await canAccessFlow(d1, 'flow-1', 'editor-1')).toEqual({ canEdit: true, canDelete: false })
  })
  it('denies access to strangers', async () => {
    expect(await canAccessFlow(d1, 'flow-1', 'stranger-1')).toEqual({ canEdit: false, canDelete: false })
  })
  it('treats flows without project_id as owner-only', async () => {
    expect(await canAccessFlow(d1, 'flow-2', 'editor-1')).toEqual({ canEdit: false, canDelete: false })
  ...
  it('treats soft-deleted flows as not accessible', async () => {
    expect(await canAccessFlow(d1, 'flow-3', 'owner-1')).toEqual({ canEdit: false, canDelete: false })
    expect(await canAccessFlow(d1, 'flow-3', 'editor-1')).toEqual({ canEdit: false, canDelete: false })
  })
  it('returns no access for non-existent flow', async () => {
    expect(await canAccessFlow(d1, 'flow-none', 'owner-1')).toEqual({ canEdit: false, canDelete: false })
  })
})
