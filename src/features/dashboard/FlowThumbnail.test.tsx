...
describe('FlowThumbnail', () => {
  ...
  it('should render an SVG element', () => {
    const { container } = render(<FlowThumbnail themeId="cloud" laneCount={4} nodeCount={12} />)
    const svg = container.querySelector('svg')
    expect(svg).toBeInTheDocument()
  })

  it('should render correct number of lane groups', () => {
    const { container } = render(<FlowThumbnail themeId="cloud" laneCount={3} nodeCount={8} />)
  ...
  it('should render with midnight theme background', () => {
    const { container } = render(<FlowThumbnail themeId="midnight" laneCount={3} nodeCount={6} />)
  ...
  it('should render with blueprint theme background', () => {
    const { container } = render(<FlowThumbnail themeId="blueprint" laneCount={3} nodeCount={6} />)
    const bgRect = container.querySelector('svg > rect')
    expect(bgRect).toHaveAttribute('fill', '#E8EDF4')
  })

  it('should render with cloud theme background for unknown theme', () => {
    const { container } = render(<FlowThumbnail themeId="unknown" laneCount={3} nodeCount={6} />)
    const bgRect = container.querySelector('svg > rect')
    expect(bgRect).toHaveAttribute('fill', '#EAEAF2')
  })

  it('should cap lanes at 5', () => {
    const { container } = render(<FlowThumbnail themeId="cloud" laneCount={8} nodeCount={20} />)
  ...
  it('should handle zero lanes gracefully', () => {
    const { container } = render(<FlowThumbnail themeId="cloud" laneCount={0} nodeCount={0} />)
    const svg = container.querySelector('svg')
    expect(svg).toBeInTheDocument()
  ...
  it('should handle zero nodes gracefully', () => {
    const { container } = render(<FlowThumbnail themeId="cloud" laneCount={3} nodeCount={0} />)
    const svg = container.querySelector('svg')
    expect(svg).toBeInTheDocument()
  })

  it('should not render arrows group when only 1 lane', () => {
    const { container } = render(<FlowThumbnail themeId="cloud" laneCount={1} nodeCount={3} />)
