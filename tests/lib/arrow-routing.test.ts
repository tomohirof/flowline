...
describe('exitPt', () => {
  ...
  it('dy がちょうど RH*0.3 の境界付近では浮動小数点の影響で下部から出る', () => {
    ...
    const c = { x: 100, y: 100 }, o = { x: 300, y: 100 + threshold }
    expect(exitPt(c, o, hw, hh, RH)).toEqual({ x: 100, y: 128 })
  })
...
describe('entryPt', () => {
  ...
  it('下方向からの矢印（dy > RH*0.3）は下部に接続', () => {
    const c = { x: 100, y: 100 }, o = { x: 100, y: 200 }
    expect(entryPt(c, o, hw, hh, RH)).toEqual({ x: 100, y: 128 })
  })

  it('水平方向は左側に接続', () => {
    const c = { x: 300, y: 100 }, o = { x: 100, y: 110 }
...
describe('buildArrowPath', () => {
  it('垂直の直線パス（dx < 2）', () => {
    const s = { x: 100, y: 128 }, e = { x: 100, y: 172 }
    const fc = { x: 100, y: 100 }, tc = { x: 100, y: 200 }
  ...
  it('水平の直線パス（dy < 2）', () => {
    const s = { x: 176, y: 100 }, e = { x: 224, y: 100 }
    const fc = { x: 100, y: 100 }, tc = { x: 300, y: 100 }
    expect(buildArrowPath(s, e, fc, tc)).toEqual({
      d: 'M176,100 L224,100',
      mx: 200, my: 100,
    })
  ...
  it('両方が縦出口の場合はZ字パス', () => {
    const s = { x: 100, y: 128 }, e = { x: 300, y: 172 }
    const fc = { x: 100, y: 100 }, tc = { x: 300, y: 200 }
    expect(buildArrowPath(s, e, fc, tc)).toEqual({
  ...
  it('両方が横出口の場合はZ字パス', () => {
    const s = { x: 176, y: 100 }, e = { x: 224, y: 200 }
    const fc = { x: 100, y: 100 }, tc = { x: 300, y: 200 }
    expect(buildArrowPath(s, e, fc, tc)).toEqual({
  ...
  it('縦出口→横入口のL字パス', () => {
    const s = { x: 100, y: 128 }, e = { x: 224, y: 200 }
    const fc = { x: 100, y: 100 }, tc = { x: 300, y: 200 }
    expect(buildArrowPath(s, e, fc, tc)).toEqual({
  ...
  it('横出口→縦入口のL字パス', () => {
    const s = { x: 176, y: 100 }, e = { x: 300, y: 172 }
    const fc = { x: 100, y: 100 }, tc = { x: 300, y: 200 }
    expect(buildArrowPath(s, e, fc, tc)).toEqual({
  ...
  // エッジケース
  it('完全に同じ始点と終点', () => {
    const s = { x: 100, y: 100 }, e = { x: 100, y: 100 }
    const fc = { x: 100, y: 80 }, tc = { x: 100, y: 120 }
    expect(buildArrowPath(s, e, fc, tc)).toEqual({
      d: 'M100,100 L100,100',
      mx: 100, my: 100,
    })
